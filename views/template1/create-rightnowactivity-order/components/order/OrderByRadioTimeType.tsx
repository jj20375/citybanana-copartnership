 ...
 */
const OrderByRadioTimeType = memo(
    ({ lng, register, label, value, setValue, required, canChooseTime }: { lng: string; register: UseFormRegister<RightNowActivityOrderCreateFormInterface>; label: Path<RightNowActivityOrderCreateFormInterface>; value: string; setValue: Function; required: boolean; canChooseTime: boolean }) => {
        ...
        const timeTypes = useMemo(() => {
            // 判斷有可使用指定時間店家時 才回傳可指定時間 radio 選項
            if (canChooseTime) {
                return defaultTimTypes;
            }
            return defaultTimTypes.filter((item) => item.value === "now");
        }, [canChooseTime]);
